...
public class TuioClient implements OSCListener {
	...
	public void connect() {
		...
		try {
			...
			connected = true;
			//System.out.println("TuioClient connected to port "+port);
	...
	public void acceptMessage(Date date, OSCMessage message) {
		...
		if (address.equals("/tuio/2Dobj")) {
			...
			} else if (command.equals("aliveOBJ")) {
			...
			} else if (command.equals("fseqOBJ")) {
			...
			if (command.equals("gyro")) {
				
				float deltaRotationVectorX = (float) args[1];
				float deltaRotationVectorY = (float) args[2];
				float deltaRotationVectorZ = (float) args[3];				

				locationX = (float) deltaRotationVectorX;
				locationY = (float) deltaRotationVectorY;
				locationZ = (float) deltaRotationVectorZ;
			}
